...
export function getArcaSearchUrl(keyword: string): string {
  ...
  return `${ARCA_BASE_URL}?target=all&keyword=${encodedKeyword}`
}
...
export async function addArcaLinks(): Promise<void> {
  ...
  for (const selector of REALTIME_SELECTORS) {
    ...
    if (items.length > 0) {
      ...
      console.log(`${LOG_PREFIX} 선택자 "${selector}"로 ${items.length}개 항목 발견`)
      break
    }
  }

  if (realtimeItems.length === 0) {
    const allSections = document.querySelectorAll('section, div[class*="section"], aside')
    for (const section of allSections) {
      const heading = section.querySelector('h2, h3, h4, .title, .heading')
      if (
        heading &&
        (heading.textContent?.includes('실시간') || heading.textContent?.includes('인기'))
      ) {
        const links = section.querySelectorAll('a')
        if (links.length > 0) {
          realtimeItems = links
          usedSelector = '텍스트 기반 검색'
          console.log(`${LOG_PREFIX} 텍스트 기반으로 ${links.length}개 항목 발견`)
          break
        }
      }
    }
  ...
  if (realtimeItems.length === 0) {
    ...
    console.log(`${LOG_PREFIX} 개발자 도구(F12)로 DOM 구조를 확인하고 선택자를 업데이트해주세요.`)
    return
  }
  ...
  console.log(`${LOG_PREFIX} ${addedCount}개 항목에 링크 추가 완료 (선택자: ${usedSelector})`)
}
